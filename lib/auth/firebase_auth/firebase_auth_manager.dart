  ...
  Future resetPassword({
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text(
                'Email ou senha inválido, senha deve conter mínimo 6 dígitos.')),
      );
      return null;
    }
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('Reset de senha enviado por email')),
    );
  }
  ...
  Future<BaseAuthUser?> _signInOrCreateAccount(
    ...
    try {
      ...
      return userCredential == null
          ? null
          : AppSifatFirebaseUser.fromUserCredential(userCredential);
    } on FirebaseAuthException catch (e) {
      final errorMsg = switch (e.code) {
        'email-already-in-use' => 'Email já cadastrado',
        'INVALID_LOGIN_CREDENTIALS' => 'Erro de credencial',
        _ => 'Email ou senha inválido, senha deve conter mínimo 6 dígitos.',
      };
      ScaffoldMessenger.of(context).hideCurrentSnackBar();
      ScaffoldMessenger.of(context).showSnackBar(
